...
expect( 'WCPOS\\WooCommercePOS\\MollieTerminal\\init' === $hooks['plugins_loaded'][11][0], 'legacy init priority unchanged' );
$registered = array();
...
expect( array( array( Settings::GATEWAY_ID, Mollie_Server_Provider::class ) ) === $registered, 'correct gateway and adapter' );
expect( true === Registration::register() && 1 === count( $registered ), 'registration idempotent' );
$supported = false;
$requires_calls = array();
call_user_func( $activation );
expect( 2 === count( $requires_calls ) && realpath( __DIR__ . '/../../mollie-terminal-for-woocommerce.php' ) === $requires_calls[1][1], 'activation hook checks Pro after PHP check' );
